...
int main(int argc, char *argv[]) {
  ...
  // Allocate the image
  float *image     = malloc(sizeof(float)*(nx+2)*(ny+16));
  float *tmp_image = malloc(sizeof(float)*(nx+2)*(ny+16));

  // Set the input image
  init_image(nx+2, ny+8, image, tmp_image);

  // Call the stencil kernel
  double tic = wtime();
  for (int t = 0; t < niters; ++t) {
    stencil(nx+2, ny+8, image, tmp_image);
    stencil(nx+2, ny+8, tmp_image, image);
  }
  double toc = wtime();
  ...
  output_image(OUTPUT_FILE, nx+2, ny+8, image);
  free(image);
}
...
void init_image(const int nx, const int ny, float * image, float * tmp_image) {
  ...
  for (int j = 0; j < 8; ++j) {
    for (int i = 0; i < 8; ++i) {
      for (int jj = j*(ny-8)/8; jj < (j+1)*(ny-8)/8; ++jj) {
        for (int ii = i*(nx-2)/8; ii < (i+1)*(nx-2)/8; ++ii) {
          if ((i+j)%2)
            image[(jj+4)+(ii+1)*ny] = 100.0;
